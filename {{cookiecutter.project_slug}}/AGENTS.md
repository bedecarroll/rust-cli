...
5. Stay on the pinned Rust toolchain (`rust-toolchain.toml`) to guarantee edition 2024 compatibility and MSRV {{ cookiecutter.rust_msrv }} features.

## Tooling

- Tool versions are pinned in `mise.toml`. Do not install tools globally.
- `rust-toolchain.toml` keeps `rustfmt`, `clippy`, and `llvm-tools-preview` aligned at Rust {{ cookiecutter.rust_toolchain }} (edition 2024). Avoid overriding the toolchain without discussion.
...
- User configuration lives in `$XDG_CONFIG_HOME/{{ cookiecutter.project_slug }}/config.toml` (falling back to `~/.config/...`). Additional overrides load from the `conf.d/` directory in lexical order.
- Honour the `--config-dir` flag or `{{ cookiecutter.project_slug | replace('-', '_') | upper }}_CONFIG_DIR` when scripting automated workflows.
